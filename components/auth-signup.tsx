...
import { Brain, Mail, Lock, CheckCircle2 } from "lucide-react"
...
export default function AuthSignup({ onComplete, onSwitchToLogin }: AuthSignupProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Local-only demo signup
      const user = { email, createdAt: new Date().toISOString() }
      localStorage.setItem("mindbuddy-user", JSON.stringify(user))
      onComplete()
    } finally {
    ...
    <div className="min-h-screen flex items-center justify-center p-6">
      <Card className="w-full max-w-md glass-strong rounded-2xl p-6">
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-semibold bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">MindBuddy</h2>
